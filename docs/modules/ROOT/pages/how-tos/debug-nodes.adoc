...
----
nodename=<NODE> <1>
oc --as=system:admin -n syn-debug-nodes debug "node/${nodename}" <2>
----
<1> The name of the node to debug.
Use the names shown in `oc get nodes`.
<2> Impersonating `system:admin` with `--as=system:admin` ensures that you have sufficient permissions to schedule the debug pod.
