...
* <<step-12,Then I compile the cluster catalog>>
...
so that corresponding resources can be decommissioned by the controller.
...
exo storage rb -r -f "${INPUT_commodore_cluster_id}-cluster-backup" || true
