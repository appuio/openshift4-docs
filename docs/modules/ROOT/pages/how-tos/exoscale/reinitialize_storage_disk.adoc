...
[source,bash]
----
node_name=$(kubectl --as=system:admin get pv ${pv_name} \
...
[source,bash]
----
kubectl --as=system:admin -n syn-rook-ceph-cluster exec -it deploy/rook-ceph-tools -- \
  ceph osd set noout
----
...
[source,bash]
----
kubectl --as=system:admin -n syn-rook-ceph-operator scale --replicas=0 \
...
[source,bash]
----
diskmaker_pod=$(kubectl --as=system:admin -n openshift-local-storage get pods \
...
[source,bash]
----
ceph_image=$(kubectl --as=system:admin -n syn-rook-ceph-cluster get job \
  -l "ceph.rook.io/pvc=${pvc_name}" \
  -o jsonpath='{.items[0].spec.template.spec.containers[0].image}')
kubectl --as=system:admin run -n syn-rook-ceph-cluster \
...
[source,bash]
----
disk_path=$(kubectl --as=system:admin get pv "${pv_name}" -o jsonpath='{.spec.local.path}')
kubectl --as=system:admin -n openshift-local-storage exec -it "${diskmaker_pod}" -- \
...
[source,bash]
----
kubectl --as=system:admin -n syn-rook-ceph-operator scale --replicas=1 \
  deploy/rook-ceph-operator
----
