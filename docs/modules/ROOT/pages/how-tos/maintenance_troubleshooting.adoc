...
[source,console]
----
kubectl --as=system:admin get clusterversions.config.openshift.io version -o jsonpath={.status.conditions}  | jq .
----
...
[source,console]
----
kubectl --as=system:admin -n openshift-machine-config-operator logs deployment/machine-config-operator -f
kubectl --as=system:admin -n openshift-machine-config-operator logs deployment/machine-config-controller -f
----
...
[source,console]
----
$ oc --as=system:admin adm release info 4.8.28
Name:      4.8.28
...
[source,console]
----
POOL_COUNT=$(kubectl --as=system:admin -n openshift-machine-config-operator get machineconfigpool --no-headers | wc -l)
kubectl --as=system:admin -n openshift-machine-config-operator get machineconfig \
...
[source,console]
----
kubectl --as=system:admin get nodes -ocustom-columns="NAME:.metadata.name,Current Config:.metadata.annotations.machineconfiguration\.openshift\.io/currentConfig,Desired Config:.metadata.annotations.machineconfiguration\.openshift\.io/desiredConfig"
----
...
----
NODE=<node-name>
POD=$(kubectl --as=system:admin -o jsonpath='{.items[0].metadata.name}' \
  -n openshift-machine-config-operator get pods \
  --field-selector="spec.nodeName=${NODE}" -l k8s-app=machine-config-daemon)
kubectl --as=system:admin -n openshift-machine-config-operator \
...
[source,console]
----
oc --as=system:admin adm drain <node-name> --delete-emptydir-data --ignore-daemonsets --force --grace-period=0
----
+
