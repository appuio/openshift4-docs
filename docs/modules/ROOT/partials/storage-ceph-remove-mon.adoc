...
for node in $(echo -n {mon-replace-list}); do
  mon_id=$(kubectl --as=system:admin -n syn-rook-ceph-cluster get pods \
...
----
for node in $(echo -n {mon-replace-list}); do
  kubectl --as=system:admin cordon "${node}"
done
----
+
...
----
mon_id=<MON_ID>
kubectl --as=system:admin -n syn-rook-ceph-cluster delete pod \
...
[source,bash]
----
kubectl --as=system:admin -n syn-rook-ceph-cluster get deploy -l app=rook-ceph-mon
----
