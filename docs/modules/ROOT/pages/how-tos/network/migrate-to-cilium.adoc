...
kubectl --as=system:admin -n syn patch apps openshift4-nodes --type=json \
  -p '[{"op":"replace", "path":"/spec/syncPolicy", "value": {}}]'
kubectl --as=system:admin -n syn patch apps openshift-upgrade-controller --type=json \
  -p '[{"op":"replace", "path":"/spec/syncPolicy", "value": {}}]'
----
...
[source,bash]
----
kubectl --as=system:admin patch clusterversion version \
  --type=merge \
  -p '
  {"spec":{"overrides":[
    {
...
[source,bash]
----
kubectl --as=system:admin -n openshift-network-operator \
...
kubectl --as=system:admin patch $mcp --type=merge -p '{"spec": {"paused": true}}'
done
----
...
kubectl --as=system:admin -n openshift-dns exec ds/node-resolver -- dig +short api-int.${CLUSTER_DOMAIN}
----
+
[NOTE]
====
...
[source,bash]
----
kubectl --as=system:admin patch network.config cluster \
  --type=merge -p '{"spec":{"networkType":"Cilium"},"status":null}'
kubectl --as=system:admin patch network.operator cluster \
...
[source,bash]
----
kubectl --as=system:admin apply -n cilium -Rf catalog/manifests/cilium/
----
+
[source,bash]
----
...
[source,bash]
----
kubectl --as=system:admin -n syn-espejo apply -f catalog/manifests/networkpolicy/10_default_networkpolicies.yaml
----
...
[source,bash]
----
kubectl --as=system:admin -n openshift-multus rollout restart ds/multus
----
====
+
[TIP]
...
Use `kubectl --as=system:admin drain --ignore-daemonsets --delete-emptydir-data --force --disable-eviction` to circumvent PDB violations if necessary.
