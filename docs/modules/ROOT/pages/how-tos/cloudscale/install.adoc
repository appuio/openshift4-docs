...
yq w -i "${CLUSTER_ID}.yml" "classes[+]" "components.openshift4-cloudscale"

yq w -i "${CLUSTER_ID}.yml" parameters.openshift4_cloudscale.variables.base_domain "${BASE_DOMAIN}"

yq w -i "${CLUSTER_ID}.yml" "parameters.openshift4_cloudscale.variables.ssh_keys[+]" "$(cat ~/.ssh/id_ed25519.pub)"

yq w -i "${CLUSTER_ID}.yml" parameters.openshift4_cloudscale.variables.ignition_ca "${CA_CERT}"
