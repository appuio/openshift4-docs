...
[source,bash]
----
kubectl --as=system:admin -n openshift-logging get sa --show-labels
----
...
[source,bash]
----
kubectl --as=system:admin -n openshift-logging delete sa elasticsearch-operator
----
...
[source,bash]
----
kubectl --as=system:admin scale deployment -n openshift-operator-lifecycle-manager olm-operator --replicas=0
kubectl --as=system:admin delete operators.operators.coreos.com elasticsearch-operator.openshift-logging
kubectl --as=system:admin scale deployment -n openshift-operator-lifecycle-manager olm-operator --replicas=1
----
