...
mc alias set "${CLUSTER_ID}" "https://object.storage.${REGION}.onstackit.cloud" "$ACCESS_KEY" "$SECRET_KEY"
...
--from-literal=REGISTRY_STORAGE_S3_ACCESSKEY=$(mc alias list ${CLUSTER_ID} -json | jq -r .accessKey) \
--from-literal=REGISTRY_STORAGE_S3_SECRETKEY=$(mc alias list ${CLUSTER_ID} -json | jq -r .secretKey)
----
+
