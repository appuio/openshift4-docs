...
kubectl wait --for=delete loadbalancers -A --all --timeout=120s
...
mc rb "${INPUT_commodore_cluster_id}/${INPUT_commodore_cluster_id}-bootstrap-ignition" --force || true
...
# Delete object user
objuser=$(jq -r -n --argjson resp "$response" '$resp.href')
echo "Deleting object user '$objuser' ..."
curl -fsSL -H "Authorization: Bearer ${INPUT_cloudscale_token}" -X DELETE "$objuser"
...
set -euo pipefail
echo "Deleting cluster on Lieutenant at ${INPUT_commodore_api_url} ..."
export COMMODORE_API_URL="${INPUT_commodore_api_url}"
curl -H "Authorization: Bearer $(commodore fetch-token)" -XDELETE \
