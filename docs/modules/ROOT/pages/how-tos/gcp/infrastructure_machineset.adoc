...
https://docs.openshift.com/container-platform/latest/machine_management/creating-infrastructure-machinesets.html[Creating infrastructure MachineSets] of the OpenShift documentation explains how to so.
For GCP, this doesn't work as documented and or the documentation is missing essential details.
This how to tries to fill the gaps but must leave open a some questions.
--
...
. Crate MachineSet for the custom node group
+
[NOTE]
====
...
As a result, the crated VM isn't allowed to access the machine config endpoint and thus fails to load its ignition configuration.
...
There content is fairly specific to a specific cluster and contains things that needs to be updated under certain conditions (for example the cluster CA certificate).
This suggest, that creating them manually doesn't seemed to be intended.
...
Any changes made to the get reverted immediately.
