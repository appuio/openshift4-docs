= Enable `LoadBalancer` serivces on cloudscale.ch

[abstract]
--
Steps to enable `LoadBalancer` serivces on an OpenShift 4 cluster from https://cloudscale.ch[cloudscale.ch].
--
...
. Create a ticket at cloudscale.ch to make the range available as a Floating Network in the project of the cluster.
    ...
    address-pools:
      - name: default
        ...
        addresses:
          - 203.0.113.128/25 <2>
----
<1> IP of the loadbalancers
<2> Public IP range chosen
...
It the namespace doesn't allow access from external nodes, everything will seem to work, but you can't access the service.
--
...
One thing to consider it, that Cilium doesn't pick up updates of the `bgp-config` ConfigMap.
