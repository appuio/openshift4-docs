...
oc --as=system:admin get oauthaccesstokens | grep ${username} | cut -d' ' -f1 | \
  xargs oc --as=system:admin delete oauthaccesstokens
----
...
oc --as=system:admin delete identity --field-selector=user.name="<username>"
oc --as=system:admin delete user "<username>"
----
