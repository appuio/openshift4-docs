...
. Enable the cloudscale loadbalancer controller
+
[source,bash]
----
yq eval -i '.applications += ["cloudscale-loadbalancer-controller"]' ${CLUSTER_ID}.yml

git commit -a -m "Enable cloudscale loadbalancer controller for ${CLUSTER_ID}"
----
endif::[]
...
yq eval -i '.parameters.cilium.olm.generate_olm_deployment = true' ${CLUSTER_ID}.yml
