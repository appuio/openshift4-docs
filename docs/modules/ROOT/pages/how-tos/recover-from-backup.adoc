...
export RESTIC_PASSWORD=$(kubectl --as=system:admin -n syn-cluster-backup get secret objects-backup-password -o jsonpath='{.data.password}' | base64 --decode)
export AWS_ACCESS_KEY_ID=$(kubectl --as=system:admin -n syn-cluster-backup get secret objects-backup-s3-credentials -o jsonpath='{.data.username}' | base64 --decode)
export AWS_SECRET_ACCESS_KEY=$(kubectl --as=system:admin -n syn-cluster-backup get secret objects-backup-s3-credentials -o jsonpath='{.data.password}' | base64 --decode)
----
...
[source,console]
----
kubectl --as=system:admin apply -f <path/to/file>
----

.Apply all files within a directory
[source,console]
----
kubectl --as=system:admin apply -Rf <path/to/dir>
----
...
[NOTE]
====
To restore Persistent Volumes (PVs) and Persistent Volume Claims (PVCs) on Cloudscale, follow the instructions in xref:how-tos/cloudscale/restore-pv-pvc-from-object-backup.adoc[].
====
