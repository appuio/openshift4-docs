...
* &nbsp;&nbsp;<<step-17,And I configure Terraform for Cloudscale>>
* <<step-18,Then I provision the loadbalancers>>
* &nbsp;&nbsp;<<step-19,And I provision the bootstrap node>>
* &nbsp;&nbsp;<<step-20,And I store the subnet ID and floating IP in the Syn hierarchy>>
* &nbsp;&nbsp;<<step-21,And I provision the control plane>>
* <<step-22,Then I deploy initial manifests>>
* &nbsp;&nbsp;<<step-23,And I wait for bootstrap to complete>>
* <<step-24,Then I remove the bootstrap node>>
* &nbsp;&nbsp;<<step-25,And I configure initial deployments>>
* &nbsp;&nbsp;<<step-26,And I wait for installation to complete>>
* <<step-27,Then I synthesize the cluster>>
* <<step-28,Then I set acme-dns CNAME records>>
* &nbsp;&nbsp;<<step-29,And I verify emergency access>>
* &nbsp;&nbsp;<<step-30,And I configure the cluster alerts>>
* &nbsp;&nbsp;<<step-31,And I enable Opsgenie alerting>>
* &nbsp;&nbsp;<<step-32,And I verify the image registry config>>
...
{{ .commodore_cluster_id }} and {{ .commodore_cluster_id }}_floaty on control.cloudscale.ch/service/<your-project>/api-token.

This step currently does not validate whether the tokens have write permission.
...
zones=$(curl -sH "$auth_header" https://api.cloudscale.ch/v1/custom-images | \
  jq --arg slug "$image_slug" -r '.[] | select(.slug == $slug) | [.zones[]|.slug]|join(" ")')
if [ -n "$zones" ] && [[ "$zones" == *"${INPUT_csp_region}1"* ]]; then
  echo "Image '$image_slug' already exists in Cloudscale ${INPUT_csp_region}, skipping upload."
  exit 0
elif [ -n "$zones" ]; then
  echo "Image 'rhcos-${INPUT_image_major}.${INPUT_image_minor}' already exists in Cloudscale ${zones}, configuring multi-zone image upload"
...
Review the file ./inventory/classes/{{ .commodore_tenant_id }}/{{ .commodore_cluster_id }}.yml.
...
echo "Commit changes"
git commit -a -m "Set cloudscale specific values in cluster ${CLUSTER_ID}"
git push
...
cluster bootstrap, such as enabling proxy protocol on the Ingress
controller, and scheduling the ingress controller on the infrastructure
nodes.
...
# Set insecure-skip-tls-verify=true in kubeconfig2 so we can use it to
# wait for the kube-apiserver CO
yq -i e '.clusters[0].cluster.ignore-skip-tls-verify = true' "${INPUT_kubeconfig_path}2"
