...
Feedback from clusters with the https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html/security_and_compliance/compliance-operator[OpenShift Compliance Operator] addon is continuously collected and generic findings are used to improve the security of all VSHN Managed OpenShift clusters.

Customers can request to have the Compliance Operator addon installed in their cluster to get detailed and tailored information about the compliance status of their cluster.
...
Please note that this is a platform backup and doesn't include application data.
