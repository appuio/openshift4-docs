...
kubectl get node -ojson | \
  jq -r '.items[] 
    ...
    .metadata.name' | \
  xargs -I {} kubectl label --as=system:admin \
...
for node in $(
  ...
  kubectl drain --as=system:admin --ignore-daemonsets \
...
+
include::partial$exoscale/ccm-iam.adoc[]
...
# Enable component
yq eval -i ".applications += [\"exoscale-cloud-controller-manager\"]" \
  ${CLUSTER_ID}.yml
...
git commit -a -m "Deploy Exoscale CCM on cluster ${CLUSTER_ID}"
git push
...
[source,bash]
----
kubectl --as=system:admin patch infrastructure.config  cluster --type=merge -p '{"spec":{"platformSpec":{"external":{"platformName":"exoscale"},"type":"External"}}}'
----
+
[source,bash]
----
kubectl --as=system:admin patch infrastructure.config cluster --type=merge --subresource=status \
...
. Check if instance-type is applied to verify the CCM is working
+
[source,bash]
----
