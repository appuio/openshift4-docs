...
exo storage upload "target/bootstrap.ign" "sos://${CLUSTER_ID}-bootstrap" --acl public-read
...
yq eval -i ".parameters.openshift4_terraform.terraform_variables.rhocs_template = \"${TF_VAR_rhcos_template}\"" \
  ${CLUSTER_ID}.yml
...
[source,console]
----
API_URL=$(yq e '.clusters[0].cluster.server' $(pwd)/../../../target/auth/kubeconfig)
