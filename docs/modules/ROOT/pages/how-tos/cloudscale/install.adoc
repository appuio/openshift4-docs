...
[CAUTION]
This guide describes the installation with https://cilium.io/[Cilium] as the network provider.
Cilium is an optional add-on and marked sections should be skipped for clusters which don't require Cilium.
Optional sections are marked with _Cilium Optional_.
